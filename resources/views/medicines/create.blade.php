                ...
                <form method="POST" action="{{ route('medicines.store') }}">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="stored_date" class="form-control @error('stored_date') is-invalid @enderror" value="{{ old('stored_date', \App\Helpers\DateHelper::formatDate(now())) }}" placeholder="DD/MM/YYYY" inputmode="numeric" pattern="\d{2}/\d{2}/\d{4}" required>
                            <div class="form-text">Use DD/MM/YYYY format.</div>
                            @error('stored_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Expiry Date</label>
                            <input type="text" name="expiry_date" class="form-control @error('expiry_date') is-invalid @enderror" value="{{ old('expiry_date') }}" placeholder="DD/MM/YYYY" inputmode="numeric" pattern="\d{2}/\d{2}/\d{4}" required>
                            <div class="form-text">Use DD/MM/YYYY format.</div>
